...
const Score = ({ category }) => {
  ...
  return (
    <div className="score-wrapper" style={{backgroundImage: `url(${backgrounds[category.number - 1]})`}}>
      <div className="score-heading">
        <img src={smallIcons[category.number - 1]} alt="small-icon" />
        <h2>{category.name}</h2>
      </div>
      <div className="score-points" style={{backgroundColor: `${colors[category.number - 1]}`}}>Rezultat: {900}</div>
      <button style={{backgroundColor: `${colors[category.number - 1]}`}} onClick={handleNavigate}>Nazad na igru</button>
